...
- **최초 1회 수동 전환**: blue/green 도입 전에는 컨테이너명이 `academy-backend-prod`(서비스명 `backend`) 하나였다. 새 워크플로가 처음 배포하기 전 서버에서 구 컨테이너를 정리해야 한다(`docker stop academy-backend-prod && docker rm academy-backend-prod`) — 안 그러면 신규 `backend_blue`가 뜰 때 orphan 컨테이너로 남는다. Caddyfile을 바꾼 뒤에는 `docker exec academy-caddy-prod caddy reload --config /etc/caddy/Caddyfile --adapter caddyfile`로 무중단 리로드한다(컨테이너 재시작은 그 순간 API/프론트 프록시 전체가 끊긴다).
