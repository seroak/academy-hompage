...
describe('PreferredSlotDto', () => {
  function makeSlot(
    overrides: Partial<{ dayOfWeek: string; startMinute: number; endMinute: number }> = {},
  ) {
  ...
  it('종료 시각이 시작 시각과 같으면 실패한다', async () => {
  ...
  it('종료 시각이 시작 시각보다 이르면 실패한다', async () => {
    const errors = await validate(makeSlot({ startMinute: 720, endMinute: 710 }));
