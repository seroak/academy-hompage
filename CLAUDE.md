...
- Meta의 Access Token Debugger가 "Malformed access token"이라고 보고해도 토큰 자체가 손상된 게 아니라, 사용자가 터미널에서 `printenv` 등으로 값을 복사·붙여넣는 과정에서 개행·공백이 섞여 든 것일 수 있다 — 웹 Debugger에 수동으로 붙여넣은 결과만 믿지 말고, 컨테이너 내부에서 프로덕션과 동일한 경로로 `debug_token` 엔드포인트를 직접 호출해 검증한다.
...
- Meta 광고 게재에는 Facebook 페이지가 필수다. Instagram 전용으로 운영하고 싶어도 페이지 생성 자체는 우회할 수 없다.
...
- `backend/.env`, `frontend/.env` 등 시크릿 파일은 통째로 `cat`/Read하지 않는다. 특정 키 값 확인이 필요하면 `grep '^KEY_NAME='`로 개별 조회한다 — 이 프로젝트에서 파일 전체 노출로 Google OAuth secret·Resend API 키·JWT_SECRET·관리자 시드 비밀번호가 대화에 유출된 사고가 실제로 있었다.
