...
- **`matchMedia` 기반 반응형 분기는 마운트 후에만 값이 확정된다**: `useIsNarrow` 같은 훅은 SSR 안전을 위해 초기값을 고정(`false`)해 두고 `useEffect`에서 실제 값으로 갱신하므로, `page.goto` 직후 곧바로 `page.evaluate`로 레이아웃 폭 등을 측정하면 하이드레이션 타이밍에 따라 전환 전 상태(SSR 초기값)를 잰 채로 우연히 통과할 수 있다(실제로 겪음 — 가로 스크롤이 사라져야 할 레이아웃 변경 후에도 옛 assertion이 초기값 기준으로 계속 통과했었다). 그 갱신에 의존하는 마크업(요일 탭 등)이 실제로 나타날 때까지 `expect(locator).toBeVisible()`로 기다린 뒤 측정한다.
...
- **도메인 변경 시 갱신할 값이 최소 4곳에 나뉘어 있다**: (1) 루트 `.env.production`(`SITE_DOMAIN`/`API_DOMAIN`/`NEXT_PUBLIC_API_BASE_URL`/`NEXT_PUBLIC_SITE_URL`, Caddy용), (2) `backend/.env.production`(`CORS_ORIGIN`/`BACKEND_PUBLIC_URL`/`FRONTEND_URL`/`COOKIE_DOMAIN`, 백엔드 런타임용), (3) **Vercel 프로젝트 환경변수**(`NEXT_PUBLIC_API_BASE_URL`/`NEXT_PUBLIC_SITE_URL`) 갱신 후 재배포 — 실제 라이브 프론트는 오라클 서버가 아니라 Vercel에서 별도 빌드되므로 루트 `.env.production`을 고쳐도 반영되지 않는다, (4) **구글 OAuth 콘솔**의 승인된 리디렉션 URI·JavaScript 출처 등록. (1)(2)만 고치고 로그인이 안 된다면 (3)(4)를 빠뜨린 것부터 의심한다.
...
- 필수 동작 검증을 실행하지 못하면 구현 결과와 검증 상태를 분리해 보고하고, 미실행 검증을 남은 작업으로 명시한다. 검증하지 않은 변경을 "완료", "정상", "통과"로 표현하지 않는다.
