...
- 도구별 전역 행동 규칙은 각 진입점에만 둘 수 있다. 동기화 대상은 이 프로젝트의 기술·아키텍처·검증·작업 방식이며, 도구 전용 전역 규칙까지 두 파일에 복제하지 않는다.
...
- 응답 중 의도치 않은 제3언어가 섞이지 않도록 출력 전 확인한다. 한국어/영어 전환뿐 아니라 일본어 등 요청되지 않은 언어가 새어나오는 경우도 동일하게 방지 대상이다. 도구 호출 직전·직후의 짧은 진행 상황 서술(한두 문장짜리 전환 멘트)에서 특히 반복 관찰됐다 — 일본어 단어("良い", "テスト" 같은 한자·가나 혼입)뿐 아니라 영어 전환 멘트("Let me...", "Now let's...")도 마찬가지다. 문장 단위뿐 아니라 단어 단위(한자·가나·영어 단어 혼입 여부)까지 확인한다.
...
- "확인이 필요합니다"라고 말했으면 실제로 사용자 응답을 기다린 뒤에만 실행한다. 물어놓고 응답 없이(또는 무관한 응답에 편승해) 자체 판단으로 바로 진행하지 않는다. 질문을 던진 다음 턴에서 도구를 호출하기 전, 그 사이에 실제 사용자 텍스트 응답이 있었는지 스스로 재확인한다 — 세션을 마무리하는 요약 메시지에서 여러 확인 질문을 한 번에 나열해놓고 다음 턴에 응답 없이 바로 실행(커밋, 테스트 데이터 삭제, 서버 접속 등)하는 사고가 반복됐다.
...
- 필수 검증을 실행하지 못했으면 구현 완료와 검증 완료를 분리해 표현하고, 미실행 검증을 남은 작업으로 명시한다. 검증하지 않은 변경을 "완료", "정상", "통과"로 표현하지 않는다.
...
- 사용자가 채팅에 시크릿을 직접 입력하면 즉시 폐기를 권고하고 새 시크릿을 발급받아 설정하도록 안내한다. 기존 시크릿으로 작업을 계속 진행하지 않는다 — "지금 작업엔 영향 없으니 이어서 진행하겠습니다", "나중에 교체를 권장드립니다" 같은 문구로 권고를 우회하고 같은 값으로 후속 명령을 계속 실행하지 않는다.
...
npm test                       # 전체 테스트 (ESM이라 NODE_OPTIONS=--experimental-vm-modules 필요 — package.json 스크립트 경유. npx jest 직접 실행은 이 옵션이 빠져 실패한다)
...
- **입력 검증은 API 경계에서도 강제**: 프론트 Zod 검증과 별개로 Nest DTO에서 필수값·형식·공백 처리를 검증하고 DTO 테스트를 둔다. 브라우저 폼 검증을 우회한 요청도 잘못된 값을 저장하면 안 된다.
...
- **커스텀 `<dialog open>` 모달은 포커스 격리가 필수**: `open` 속성만으로는 초기 포커스·Tab 순환·트리거 복원이 되지 않는다. `useModalFocusTrap`을 적용하고 닫기 버튼에 `data-autofocus`를 둬 키보드 포커스가 배경으로 빠지지 않게 한다.
...
- 세로로 긴 그리드에서 터치 드래그 선택은 페이지 스크롤과 충돌한다. 셀 높이×행 수가 뷰포트보다 훨씬 크면(`PreferredSlotsPicker`의 시간표처럼) 손가락으로 셀 위를 세로로 끄는 제스처가 브라우저 스크롤로 가로채져(`pointercancel`) 드래그 선택이 성립하지 않는다. `touch-action:none`으로 스크롤을 막으면 이번엔 페이지 자체를 못 내려간다. 해법은 `pointerType` 분기다. 마우스는 기존 드래그(`setPointerCapture` + 실시간 preview)를 그대로 두고, 터치는 셀에서 캡처하지 않고 이동거리 임계값(10px)으로 탭/스크롤을 구분해 `pointerup` 시점에만 커밋하는 "탭 2번" 방식으로 분리한다(`usePreferredSlotsSelection.ts`의 `handleCellTap`/`PreferredSlotCell.tsx` 참고). 그리드 자체도 모바일에서는 폭이 아니라 높이를 줄이는 레이아웃 전환(요일 탭 + 하루씩)을 함께 고려한다. 반응형 분기가 필요하면 `frontend/src/hooks/useIsNarrow.ts`(matchMedia 기반, SSR 안전 초기값 `false`)를 재사용한다.
...
- **예약 충돌 검사는 쓰기와 같은 직렬화 트랜잭션에서 수행**: 예약 생성·이동처럼 충돌 여부를 확인한 뒤 쓰는 흐름은 Prisma `Serializable` 트랜잭션에 함께 묶고, 동시성 충돌 `P2034`만 제한 횟수로 재시도한다. 조회와 쓰기를 분리해 동시에 들어온 요청이 같은 슬롯을 통과하게 만들지 않는다.

## React 상태 관리 규칙

다음 규칙은 이 프로젝트의 모든 React 코드에 공통 적용한다.
...
- `matchMedia` 기반 반응형 분기는 마운트 후에만 값이 확정된다. `useIsNarrow` 같은 훅은 SSR 안전을 위해 초기값을 고정(`false`)해 두고 `useEffect`에서 실제 값으로 갱신하므로, `page.goto` 직후 곧바로 `page.evaluate`로 레이아웃 폭 등을 측정하면 하이드레이션 타이밍에 따라 전환 전 상태(SSR 초기값)를 잰 채로 우연히 통과할 수 있다(실제로 겪음). 그 갱신에 의존하는 마크업(요일 탭 등)이 실제로 나타날 때까지 `expect(locator).toBeVisible()`로 기다린 뒤 측정한다.
- 프론트 E2E·빌드 전후 `git status`와 diff를 비교한다. `next-env.d.ts`, `tsconfig.tsbuildinfo` 등 자동 생성 파일이 바뀌면 세션 시작 상태를 먼저 확인하고, 기능 변경이 아닌 산출물만 기존 상태로 정리한다. 기존 사용자 변경을 덮어쓰지 않는다.
...
Prisma 스키마나 마이그레이션이 바뀐 기능은 목 테스트·빌드 통과만으로 완료하지 않는다. 대상 로컬 DB에 마이그레이션을 적용하고 실제 API 요청을 최소 1회 실행해 스키마와 런타임 연결을 확인한다.
...
- 도메인 변경 시 갱신할 값이 최소 4곳에 나뉘어 있다. (1) 루트 `.env.production`(`SITE_DOMAIN`/`API_DOMAIN`/`NEXT_PUBLIC_API_BASE_URL`/`NEXT_PUBLIC_SITE_URL`), (2) `backend/.env.production`(`CORS_ORIGIN`/`BACKEND_PUBLIC_URL`/`FRONTEND_URL`/`COOKIE_DOMAIN`), (3) Vercel 프로젝트 환경변수(`NEXT_PUBLIC_API_BASE_URL`/`NEXT_PUBLIC_SITE_URL`) 갱신 후 재배포 — 실제 라이브 프론트는 Vercel에서 별도 빌드되므로 루트 `.env.production`만으로는 반영되지 않는다, (4) 구글 OAuth 콘솔의 리디렉션 URI·JavaScript 출처 등록. (1)(2)만 고치고 로그인이 안 되면 (3)(4)부터 의심한다.

## 개발 워크플로

- 백엔드 기능 추가/수정은 TDD(RED-GREEN)로 진행 — `*.spec.ts` 먼저 작성해 실패 확인 후 구현.
- 프론트·백엔드 통합 경로가 바뀐 경우 양쪽을 실행하고 실제 브라우저(Playwright) 또는 curl로 end-to-end 확인한다. 한쪽에 국한된 정적 변경까지 양쪽 서버 실행을 요구하지 않는다.
- 필수 동작 검증을 실행하지 못하면 구현 결과와 검증 상태를 분리해 보고하고, 미실행 검증을 남은 작업으로 명시한다.
- lint·정적 분석 경고 해결 요청에서 임계값 상향, 규칙 비활성화, 예외 추가는 해결로 간주하지 않는다. 설정 완화가 필요하면 코드 수정 대신 적용하기 전에 사용자 승인을 받는다.
...
- 공유 개발 DB에 E2E 테스트 데이터(부모 계정, 예약 등)가 남으면 완료 보고에 식별자를 명시하고 삭제 여부를 사용자에게 확인받는다. 테스트 전용 임시 DB를 계획대로 통째로 폐기하는 경우에는 별도 확인 없이 폐기 사실만 보고한다.
