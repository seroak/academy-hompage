...
const dashboard = {
  ...
  totals: { spendWon: 140000, impressions: 21000, linkClicks: 260, landingVisits: 210, ctaClicks: 40, formStarts: 18, submitAttempts: 9, submitBlocked: 4, submitErrors: 2, leads: 10, validLeads: 6, bookings: 4, visits: 3, registrations: 2, cpm: 6667, cpc: 538, ctr: 1.24, costPerLead: 14000, costPerValidLead: 23333, costPerRegistration: 70000, clickToLandingRate: 80.77, landingToLeadRate: 4.76, leadToValidRate: 60, validToBookingRate: 66.67, bookingToVisitRate: 75, visitToRegistrationRate: 66.67 },
  creatives: [{ campaignId: '10', campaignName: '흥덕 7월', adId: '30', adName: '수업 영상', imageUrl: 'https://example.com/30.jpg', thumbnailUrl: 'https://example.com/30-thumb.jpg', spendWon: 80000, impressions: 12000, linkClicks: 170, landingVisits: 140, ctaClicks: 28, formStarts: 12, submitAttempts: 6, submitBlocked: 3, submitErrors: 1, leads: 7, validLeads: 5, bookings: 3, visits: 2, registrations: 2, cpm: 6667, cpc: 471, ctr: 1.42, costPerLead: 11429, costPerValidLead: 16000, costPerRegistration: 40000, clickToLandingRate: 82.35, landingToLeadRate: 5, leadToValidRate: 71.43, validToBookingRate: 60, bookingToVisitRate: 66.67, visitToRegistrationRate: 100 }],
...
test('전체 전환 흐름은 광고 노출부터 등록까지 모든 단계를 표시한다', async ({ page }) => {
  await page.route(apiPattern('/marketing/dashboard(?:\\?.*)?$'), (route) => fulfillJson(route, 200, dashboard))

  await page.goto('/admin/marketing')

  const funnel = page.getByRole('region', { name: '전체 전환 흐름' })
  for (const [label, value] of [
    ...
    const step = funnel.getByRole('listitem').filter({ hasText: label })
    await expect(step.getByText(label, { exact: true })).toBeVisible()
    await expect(step.getByText(value, { exact: true })).toBeVisible()
  }
})
