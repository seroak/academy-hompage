...
export function monthColor(index: number) {
  return monthColors[index] ?? monthColors[1];
}
  ...
  return (
    <section data-testid="month-calendar" className="min-w-0">
      ...
      <div className="overflow-hidden rounded-[22px] border border-[#ead9b8] bg-[#ead9b8]">
        <table
          className="w-full table-fixed select-none border-separate border-spacing-px"
            ...
            <tr>
              {["일", "월", "화", "수", "목", "금", "토"].map((label, index) => (
                <th
                  scope="col"
                  key={label}
                  className={`bg-[#fffdf7] py-2 text-center text-xs font-black ${index === 0 ? "text-[#d44732]" : index === 6 ? "text-[#3466c2]" : "text-[#655b4c]"}`}
                >
                  {label}
                </th>
              ))}
            ...
            {weeks.map((week, weekIndex) => (
              <tr key={weekIndex}>
                {week.map((calendarDay, dayIndex) => {
                  if (!calendarDay)
                    return (
                      <td key={`blank-${weekIndex}-${dayIndex}`} aria-hidden="true" className="bg-[#fffdf7] p-0" />
                    );
                  const entry = entries.get(calendarDay.date);
                  const colorIndex = entry?.classMonth ? classMonths.indexOf(entry.classMonth) : -1;
                  const cellClass =
                    entry?.kind === "CLASS"
                      ? `${monthColor(colorIndex)} border`
                      : entry?.kind === "HOLIDAY"
                        ? "bg-[#fff0eb] border border-[#f2b8aa]"
                        : entry?.kind === "CLOSED"
                          ? "bg-[#f3eee4] border border-[#d9cdbb]"
                          : "bg-[#fffdf7] border border-transparent";
                  const weekdayClass =
                    entry?.kind === "HOLIDAY" || calendarDay.weekday === 0
                      ? "text-[#d44732]"
                  ...
                  const content = (
                    <>
                      ...
                      {entry?.note && (
                        <span className="mt-1 line-clamp-2 text-[10px] font-black leading-tight text-[#a23a2c]">
                          {entry.note}
                        </span>
                      )}
                    </>
                  ...
                  return (
                    <td
                      ...
                      aria-label={`${calendarDay.date}${entry?.note ? ` ${entry.note}` : ""}`}
                      className="p-0"
                    >
                      {onDateClick ? (
                        <button
