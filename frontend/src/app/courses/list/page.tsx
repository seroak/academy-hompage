...
export const metadata: Metadata = {
  title: '개설 강좌',
  description: '아이꿈 학원에서 현재 개설한 강좌를 확인해 보세요.',
  alternates: { canonical: siteUrl('/courses/list') },
}
...
export default async function CourseListPage() {
  ...
  return (
    <Layout>
      <div>
        <Link href="/courses" className="text-sm font-bold text-brand-700 hover:underline">
          ← 수학교육 과정 소개
        </Link>
        <h1 className="mt-4 text-2xl font-bold text-slate-900">개설 강좌</h1>
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {courses.map((course, index) => (
            ...
            >
              ...
              {course.instructor && <p className="mt-1 text-xs text-slate-500">담당: {course.instructor.name}</p>}
            </Link>
          ))}
          {courses.length === 0 && <p className="col-span-full py-10 text-center text-slate-400">등록된 강좌가 없습니다.</p>}
        </div>
      </div>
    </Layout>
  )
